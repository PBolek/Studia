...
(define (solve-ax+by=1 a b)
  (define (set-p)
    (let ((p (solve-ax+by=1 b (- a (* (floor (/ a b)) b)))))
      (cons (cdr p) (- (car p) (* (floor (/ a b)) (cdr p))))))
        (if (= b 0)
            (cons 1 0)
            (set-p)))
...
(define (encrypt-and-sign message recipient-public-key sender-private-key)
  (let ((message1 (RSA-encrypt message recipient-public-key))
        (signature (RSA-transform (compress (string->intlist message)) sender-private-key)))
    (make-signed-message message1 signature)))

(define (authenticate-and-decrypt cyphertext recipient-private-key sender-public-key)
  (let ((message1 (RSA-decrypt (message cyphertext) recipient-private-key))
        (signature (RSA-unconvert-list (list (signature cyphertext)) sender-public-key)))
        (if (= (car signature) (compress (string->intlist message1)))
         message1
         false)))
