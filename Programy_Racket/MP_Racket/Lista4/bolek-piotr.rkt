...
(define (make-node elem heap-a heap-b)
  ...
  (list
   'hnode
   elem
   (+ 1 (choose-bigger (rank heap-a) (rank heap-b)))
   (if (= (choose-bigger (rank heap-a) (rank heap-b)) (rank heap-a))
       heap-b
       heap-a)
   (if (= (choose-bigger (rank heap-a) (rank heap-b)) (rank heap-a))
       heap-a
...
(define (heap-merge h1 h2)
  (cond
   ...
   [(leaf? h2) h1]
   ;; XXX: fill in the implementation
   [else (if (= (choose-bigger (elem-priority (heap-min h1))
...
;;Testy
(sorted? (heapsort (randlist 100 100)))
(sorted? (heapsort (randlist 100 1)))
(sorted? (heapsort (randlist 10 100)))
(sorted? (heapsort (randlist 5 10)))