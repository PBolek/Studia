...
(define (lazy-let-def-cons var e)
  (list var e))
...
(define (eval-env e env)
  (cond [(const? e)
        ...
        [(lazy-let? e)
         (let ([cl (closure-cons null (lazy-let-def-expr (lazy-let-def e)) env)])
           (eval-env (lazy-let-expr e) (add-to-env (lazy-let-def-var (lazy-let-def e)) cl env)))]
        [(my-null? e)
         null]
        [(cons? e)
        ...
        [(var? e)
         (let ((env1 (find-in-env (var-var e) env)))
           (if (closure? env1)
               (apply-closure env1 null)
               env1))]
        [(lambda? e)
...
(define (test3)
  (eval '(let (x (lambda-rec (fib n) (lazy-let [f (+ (fib (- n 2)) (fib (- n 1)))]
                                               (if (= n 0) 0
                                                   (if (= n 1) 1 f)))))
           (x 5))))

(define (test4)
  (eval '(lazy-let (a 15) (let (a 2) (+ a a)))))

(define (test5)
  (eval '(lazy-let (a 15) (let (b 2) (+ a b)))))

(define (test6)
  (eval '(lazy-let (x (let (y 4) y)) (+ x 3))))

(define (test7)
  (eval '(lazy-let (x (- 2 (let (z 3) z))) (+ x 2))))
